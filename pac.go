...
var (
	charsFile = flag.String("chars-file", "chars.json", "path to custom characters file")
...
func loadchars(file string) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func main() {
	//Ask for the number of ghosts
	fmt.Println("How many ghosts do you want? (1-12)")
	fmt.Scanln(&numGhosts)
	if numGhosts < 1 || numGhosts > 12 {
		fmt.Println("Invalid number of ghosts. Exiting...")
		os.Exit(1)
	}
	flag.Parse()
	...
	err = loadchars(*charsFile)
	if err != nil {
		log.Println("failed to load charsuration:", err)
		return
	}
